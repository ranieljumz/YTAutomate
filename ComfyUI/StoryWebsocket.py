...
file_path = os.path.join(script_dir, "story_workflow.json")
try:
    with open(file_path, "r") as f:
        BASE_WORKFLOW = json.load(f)
    logger.info(
        "Loaded base workflow from story_workflow.json"
    )
except FileNotFoundError:
    logger.error(
        "Error: story_workflow.json not found. Please save your workflow in API format."
    )
